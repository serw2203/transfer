...
public class AnalyticalServiceImpl implements AnalyticalService {
    ...
    public ExtractRoot extracts(String accNum, Timestamp startDate, Timestamp stopDate) throws Exception {
        ...
        jdbc.createTrans();
        try {
            extracts.getInputs().addAll(this.analyticalDao.balance(jdbc, accNum, startDate, false));
            extracts.getOutputs().addAll(this.analyticalDao.balance(jdbc, accNum, stopDate));
